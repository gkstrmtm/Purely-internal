...
export function PortalMultiSelectDropdown(props: {
  label?: string;
  value: string[];
  ...
  const selected = useMemo(() => {
    const set = new Set((value || []).map((x) => normalizeValue(String(x))).filter(Boolean));
  ...
  const menuNode = useMemo(() => {
    ...
    const showAddCustom =
      allowCustom &&
      normalizeValue(q) &&
      !options.some((o) => o.value.toLowerCase() === normalizeValue(q).toLowerCase());
  ...
  const summary = selected.length ? `${selected.length} selected` : (label ? `Select ${label.toLowerCase()}` : "Select");
