...
function sanitizePurgeMetadata(metadata: Record<string, unknown> | null | undefined) {
  ...
  if (dependencySummary && typeof dependencySummary === "object" && !Array.isArray(dependencySummary)) {
    ...
    if (entries.length) {
      safeMetadata.dependencySummary = Object.fromEntries(entries);
    }
  }
...
export async function restoreArchivedEntity(input: {
  ...
  if (!Number(updated)) {
    return { ok: false as const, restored: false as const };
  }
...
export async function purgeArchivedEntity(input: {
  ...
  if (!Number(updated)) {
    return { ok: false as const, purged: false as const };
  }
...
export async function searchArchivedEntities(input: {
  ...
  if (ownerId) {
    whereParts.push(`archive."ownerId" = ${push(ownerId)}`);
  }

  if (input.entityType) {
    whereParts.push(`archive."entityType" = ${push(input.entityType)}`);
  }
