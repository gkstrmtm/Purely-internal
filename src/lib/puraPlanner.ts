...
export async function planPuraActions(opts: {
  ...
  const baseSystem = [
    ...
    "- IMPORTANT: If the user explicitly names multiple portal surfaces in an audit/review/readiness request, include read-only steps for each named surface in the same execute plan whenever supported actions exist.",
    "  Example: 'audit tasks, inbox, reviews, booking, reporting, and dashboard' should plan multiple inspection steps instead of a single narrow read.",
    "- IMPORTANT: For broad audits, do NOT ask the user to choose specific records or areas unless a later mutation truly requires one exact record. Pull current state first, then summarize actual weak spots from the retrieved data.",
    ...
    "  - If active funnel/page context is already present and the user refers to this page, the same page, that funnel, or asks to use an existing form on that page, stay anchored there instead of switching to unrelated discovery like inbox threads or chat-history lookups.",
    ...
    "  - For 'publish that blog post', resolve the current blog post from thread context and use blogs.posts.publish.",
    "  - For 'create a funnel called ...', use funnel.create. If slug is missing, derive it from the name instead of asking.",
