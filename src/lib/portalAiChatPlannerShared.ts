...
export function buildPlannerSystemPrompt(opts: { cheatSheet: string; extraSystem?: string | undefined | null }): string {
  return [
    ...
    "- IMPORTANT: If the user is editing a funnel/page and mentions adding or using a calendar on that page, do NOT use booking.calendars.update unless they explicitly asked to change calendar settings like duration, meeting location, or notification emails.",
    ...
    "- IMPORTANT: If the user names multiple portal surfaces in one audit/review request, inspect EACH named surface in the same work run whenever supported read-only actions exist.",
    "- IMPORTANT: For readiness audits, weak-spot reviews, or broad portal checkups, do not ask for specific records first unless a later mutating step truly requires one exact record.",
    "- IMPORTANT: If active Funnel Builder context is present and the user refers to this page, that same page, the current funnel, or the active form on that page, stay anchored to Funnel Builder instead of drifting into inbox or ai_chat thread lookups.",
...
export function buildPlannerUserPrompt(opts: {
  ...
  return [
    ...
    opts.unresolvedRun
      ...
      : null,
    opts.nextStepContext
      ? `Latest ready next step after successful work (use this when the user wants to keep going) (JSON):\n${JSON.stringify(opts.nextStepContext, null, 2).slice(0, 2500)}`
      : null,
    opts.continuationIntent && opts.unresolvedRun
      ...
      : opts.continuationIntent && opts.nextStepContext
        ? "Continuation note: the latest user request looks like a continue/resume instruction. Unless the new request clearly changes direction, keep advancing the successful thread objective and suggested next step above instead of restarting from scratch."
      : null,
    "Recent messages:",
