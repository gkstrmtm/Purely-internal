...
export async function POST(req: Request) {
  ...
  const advertiserUserId = user.id;
  ...
  const metaUserId = String(session?.metadata?.advertiserUserId ?? "").trim();
  if (metaKind !== "ads_topup" || !metaUserId || metaUserId !== advertiserUserId) {
  ...
  const amountCents = normalizeInt(session?.metadata?.amountCents);
  if (!amountCents) return NextResponse.json({ ok: false, error: "Missing amount metadata" }, { status: 400 });

  const now = new Date();
  const ledgerSlug = "ads-topup-ledger";

  const result = await prisma.$transaction(async (tx) => {
    const existing = await tx.portalServiceSetup.findUnique({
      where: { ownerId_serviceSlug: { ownerId: advertiserUserId, serviceSlug: ledgerSlug } },
    ...
    const account = await tx.adsAdvertiserAccount.upsert({
      ...
      create: { userId: advertiserUserId, balanceCents: amountCents },
      select: { id: true },
    });
    ...
    await tx.adsAdvertiserLedgerEntry.create({
      data: {
        ...
        kind: "TOPUP",
        amountCents,
        metaJson: { source: "stripe_checkout", sessionId },
      },
      select: { id: true },
    });
  ...
  return NextResponse.json({ ok: true, applied: !result.alreadyApplied, amountAddedCents: amountCents });
}
