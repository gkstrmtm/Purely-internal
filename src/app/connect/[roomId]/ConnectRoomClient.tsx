...
export function ConnectRoomClient(props: { roomId: string; signedInName?: string | null }) {
	...
	const fetchRoomSettingsOnceRef = useRef(fetchRoomSettingsOnce);
	const refreshParticipantsOnceRef = useRef(refreshParticipantsOnce);
	const startParticipantsRefreshRef = useRef(startParticipantsRefresh);
	const startPollingRef = useRef(startPolling);

	fetchRoomSettingsOnceRef.current = fetchRoomSettingsOnce;
	refreshParticipantsOnceRef.current = refreshParticipantsOnce;
	startParticipantsRefreshRef.current = startParticipantsRefresh;
			...
			if (raw) {
				...
				if (parsed?.participantId && parsed?.secret) {
					...
					void (async () => {
						...
						startParticipantsRefreshRef.current();
						void fetchRoomSettingsOnceRef.current().catch(() => null);
						await refreshParticipantsOnceRef.current().catch(() => null);
					})();
				}
			}
		} catch {
...
function VideoIcon() {
	return (
		<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-current">
			<path d="M15 10.5L20 7V17L15 13.5M5 6H13C14.1046 6 15 6.89543 15 8V16C15 17.1046 14.1046 18 13 18H5C3.89543 18 3 17.1046 3 16V8C3 6.89543 3.89543 6 5 6Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
		</svg>
	);
	}
...
function VideoOffIcon() {
	return (
		<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-current">
			<path d="M15 10.5L20 7V17L15 13.5M10.5 6H13C14.1046 6 15 6.89543 15 8V16C15 17.1046 14.1046 18 13 18H5C3.89543 18 3 17.1046 3 16V10.5M2 2L22 22M8.2 6.2C7.93333 6.06667 7.62222 6 7.26667 6H5C3.89543 6 3 6.89543 3 8V10.2667" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
		</svg>
	);
	}
