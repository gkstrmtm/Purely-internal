...
export async function GET(req: Request) {
  ...
  function hasCoverage(userId2: string, start: Date, end: Date) {
    const bs = blocksByUser.get(userId2) ?? [];
    for (const b of bs) {
  ...
  function hasConflict(userId2: string, start: Date, end: Date) {
    const cs = apptsByUser.get(userId2) ?? [];
    for (const c of cs) {
