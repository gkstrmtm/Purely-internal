...
export async function POST(req: Request) {
  ...
  for (const file of files) {
    ...
    if (buffer.length > MAX_BYTES) {
      return NextResponse.json({ ok: false, error: `"${file.name}" is too large (max 10MB)` }, { status: 400 });
    }

    const fileName = String(file.name || "upload.bin");
    const mimeType = String(file.type || "application/octet-stream").slice(0, 120);

    const mirrored = await mirrorUploadToMediaLibrary({ ownerId, fileName, mimeType, bytes: buffer });
    if (!mirrored) {
