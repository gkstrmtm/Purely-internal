...
export async function POST(req: Request) {
  ...
  if (channel === "sms") {
    ...
    if (!sendResult.ok) return NextResponse.json({ ok: false, error: sendResult.error }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: e instanceof Error ? e.message : "Email failed" }, { status: 400 });
  }
