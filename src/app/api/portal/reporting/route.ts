...
export async function GET(req: Request) {
  ...
  const [credits, aiEventsRaw, missedEventsRaw, leadRuns, bookingSite, reviewsAgg, leadsCount, contactsCount] =
    await Promise.all([
      ...
      listAiReceptionistEvents(ownerId, 200),
      listMissedCallTextBackEvents(ownerId, 200),
      ...
      prisma.portalLead.count({ where: { ownerId, createdAt: { gte: start } } }),
      prisma.portalContact.count({ where: { ownerId, createdAt: { gte: start } } }),
    ]);

  const bookingCount = bookingSite
    ? await prisma.portalBooking.count({ where: { siteId: bookingSite.id, createdAt: { gte: start } } })
    : 0;
