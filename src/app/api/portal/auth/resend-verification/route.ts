...
export async function POST() {
  ...
  const row = await prisma.user.findUnique({ where: { id: user.id }, select: { email: true, emailVerifiedAt: true } });
  ...
  const res = await sendVerifyEmail({ userId: user.id, toEmail: row.email });
