...
function wavSineTone(opts: { seconds: number; hz: number; sampleRate: number; amplitude: number }): Uint8Array {
  const seconds = Math.max(0.2, Math.min(8, Number.isFinite(opts.seconds) ? opts.seconds : 1));
  ...
  const amplitude = Math.max(0.05, Math.min(0.7, Number.isFinite(opts.amplitude) ? opts.amplitude : 0.25));
...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const bytes = wavSineTone({ seconds: 12, hz: toneHz, sampleRate: 8000, amplitude: 0.25 });
