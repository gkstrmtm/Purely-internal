...
export async function POST(req: Request) {
  ...
  const storedKb = parseKnowledgeBase((current.settings as any).voiceKnowledgeBase);
  ...
  const created = await createElevenLabsKnowledgeBaseFile({
    apiKey,
    file,
    name: `AI Receptionist - Voice - ${fileName}`.slice(0, 200),
  ...
  const nextSettings = await setAiReceptionistSettings(ownerId, { ...current.settings, voiceKnowledgeBase: nextKb as any });
