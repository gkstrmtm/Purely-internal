...
const bodySchema = z.object({
  module: z.enum(["blog", "booking", "crm"]),
  successPath: z.string().min(1).optional(),
  cancelPath: z.string().min(1).optional(),
});

function priceIdForModule(module: "blog" | "booking" | "crm") {
  ...
  return process.env.STRIPE_PRICE_CRM_AUTOMATION ?? "";
}
...
export async function POST(req: Request) {
  ...
  if (!priceId) {
    return NextResponse.json(
      { error: "That module is not for sale yet" },
      { status: 400 },
    );
  }
