...
export async function POST(req: Request, ctx: { params: Promise<{ roomId: string }> }) {
  ...
  try {
    ...
    if (room.hostParticipantId !== parsed.data.participantId) {
      return NextResponse.json({ ok: false, error: "Only the host can admit participants" }, { status: 403 });
    }
    ...
    const updated = await prisma.connectParticipant.updateMany({
      ...
      data: { status: "approved", admittedAt: new Date(), deniedAt: null },
    });
    ...
    return NextResponse.json(
      { ok: false, error: "Failed to admit participant", details: e instanceof Error ? e.message : "Unknown error" },
      { status: 500 },
    );
  }
}
