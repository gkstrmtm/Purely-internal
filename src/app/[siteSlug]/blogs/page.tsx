...
export default async function ClientBlogsIndexPage(props: PageProps) {
  ...
  const hostedBlogsIndex = await (prisma as any).hostedPageDocument.findFirst({
    where: { ownerId, service: "BLOGS", pageKey: "blogs_index" },
  ...
  const blogArchiveSection = (
    <HostedBlogArchiveSection brandName={brandName} posts={posts} page={page} pageSize={take} basePath={`/${siteHandle}/blogs`} />
  );
