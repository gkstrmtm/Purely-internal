...
export default async function PublicMediaFolderPage({
  ...
  const { folderId, token } = params;
  const apiJsonUrl = useMemo(() => `/api/public/media/folder/${String(folderId)}/${String(token)}?json=1`, [folderId, token]);
  const zipUrl = useMemo(() => `/api/public/media/folder/${String(folderId)}/${String(token)}`,
    [folderId, token],
  );
          ...
          <div className="mt-8">
            ...
            <div className="mt-2 space-y-2">
              {data.items.map((it) => {
                ...
                return (
                  <div key={it.id} className="flex items-center justify-between gap-3 rounded-2xl border border-zinc-200 bg-white px-4 py-3">
